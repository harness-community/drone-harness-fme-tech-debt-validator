...
╚══════════════════════════════════════════════════════════════════════"""
    ...
    @staticmethod
    def format_stale_flag_error(flag_name: str, threshold: str, last_activity: str, flag_type: str = "modified") -> str:
        ...
        for path, node in tree:
            ...
            if isinstance(node, javalang.tree.VariableDeclarator):
                if (isinstance(node.initializer, javalang.tree.Literal) and
                    ...
                    node.initializer.value.startswith('"') and node.initializer.value.endswith('"')):
...
class CITestRunner:
    def __init__(self):
        ...
        self.production_environment_name = os.getenv('PLUGIN_PRODUCTION_ENVIRONMENT_NAME', 'Production')
        ...
        self.flag_last_modified_threshold = os.getenv('PLUGIN_FLAG_LAST_MODIFIED_THRESHOLD', '-1')
        self.flag_last_traffic_threshold = os.getenv('PLUGIN_FLAG_LAST_TRAFFIC_THRESHOLD', '-1')
        self.flag_at_100_percent_last_modified_threshold = os.getenv('PLUGIN_FLAG_AT_100_PERCENT_LAST_MODIFIED_THRESHOLD', '-1')
        self.flag_at_100_percent_last_traffic_threshold = os.getenv('PLUGIN_FLAG_AT_100_PERCENT_LAST_TRAFFIC_THRESHOLD', '-1')
        ...
        if not self.get_flags():
            logger.error("Failed to fetch flags from Harness - some tests may not work correctly")
        
        if not self.get_feature_flags_in_code():
            logger.error("Failed to analyze code for flags - some tests may not work correctly")
    ...
    def get_flags(self):
        try:
            ...
            if not harness_project:
                logger.error(f"Project '{self.harness_project}' not found in account '{self.harness_account}'")
                return False
            ...
            try:
                ...
                for environment in environments:
                    if environment.get('name') == self.production_environment_name:
                        ...
                        logger.info(f"Found production environment: {environment.get('name')}")
    ...
    def check_if_flag_count_exceeds_limit(self) -> bool:
        if int(self.max_flags_in_project) > -1 and len(self.flags_in_code) > int(self.max_flags_in_project):
    ...
    def _check_flag_threshold(self, threshold_value: str, attribute_name: str, error_message_template: str, check_100_percent: bool = False) -> bool:
            ...
            if meta_flag:
                ...
                if tags:
                    try:
                        ...
                        if hasattr(tags, 'map') and hasattr(tags, 'any'):
                            # Use built-in methods if available
                            is_permanent = tags.map(lambda tag: getattr(tag, 'name', '').lower()).any(lambda tag: tag in self.permanent_flags_tag.lower().split(','))
                        else:
                            # Fallback for list-like tags
                            for tag in tags:
                                tag_name = getattr(tag, 'name', '')
    ...
    def run_tests(self) -> bool:
        ...
        tests = [
            (self.check_if_flags_have_remove_these_tags, "feature flag removal tag check"),
            (self.check_if_flag_count_exceeds_limit, "feature flag count check"),
            (self.check_flag_last_modified_threshold, "feature flag last modified threshold check"),
            (self.check_flag_last_traffic_recieved_threshold, "feature flag last traffic threshold check"),
            (self.check_flag_last_modified_threshold_100_percent, "feature flag last modified threshold check for 100 percent flags"),
            (self.check_flag_last_traffic_recieved_threshold_100_percent, "feature flag last traffic threshold check for 100 percent flags"),
        ]
