...
class ThresholdValidator:
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        if self.debug:
            ...
            logger.debug(f"Flag last modified threshold: '{self.flag_last_modified_threshold}' {'(DISABLED)' if self.flag_last_modified_threshold == '-1' else '(ENABLED)'}")
            logger.debug(f"Flag last traffic threshold: '{self.flag_last_traffic_threshold}' {'(DISABLED)' if self.flag_last_traffic_threshold == '-1' else '(ENABLED)'}")
            logger.debug(f"100% flag last modified threshold: '{self.flag_at_100_percent_last_modified_threshold}' {'(DISABLED)' if self.flag_at_100_percent_last_modified_threshold == '-1' else '(ENABLED)'}")
            logger.debug(f"100% flag last traffic threshold: '{self.flag_at_100_percent_last_traffic_threshold}' {'(DISABLED)' if self.flag_at_100_percent_last_traffic_threshold == '-1' else '(ENABLED)'}")
            logger.debug("=========================================")
