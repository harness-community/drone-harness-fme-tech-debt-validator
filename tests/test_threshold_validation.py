...
class TestThresholdValidator:
    ...
    def test_permanent_flag_detection_positive(self):
        ...
        flag_meta_data = {
            "test-flag": create_mock_flag_meta("test-flag", ["frontend", "permanent"])
        }
    ...
    def test_permanent_flag_detection_negative(self):
        ...
        flag_meta_data = {
            "test-flag": create_mock_flag_meta("test-flag", ["frontend", "deprecated"])
        }
    ...
    def test_permanent_flag_with_no_config(self):
        ...
        flag_meta_data = {
            "test-flag": create_mock_flag_meta("test-flag", ["permanent"])
        }
    ...
    def test_permanent_flags_excluded_from_threshold_checks(self):
        ...
        # Run threshold check
        result = self.validator.check_all_thresholds_consolidated(
            flags_in_code, flag_meta_data, flag_data
        )
    ...
    def test_permanent_flags_excluded_from_single_threshold_check(self):
        ...
        failed_flags = self.validator._run_single_threshold_check(
            flags_in_code, flag_meta_data, flag_data, "90d", "last_update_time", False
        )
    ...
    def test_multiple_permanent_tags(self):
        ...
        validator = ThresholdValidator({
            "permanent_flags_tag": "permanent,keep,core,critical",
            "flag_last_modified_threshold": "90d",
            "debug": False
        })
...
class TestPermanentFlagIntegration:
    ...
    def test_permanent_flags_excluded_from_all_threshold_types(self):
        ...
        failed_flags = self.validator._run_single_threshold_check(
            flags_in_code, flag_meta_data, flag_data, "1s", "last_update_time", False
        )
        ...
        failed_flags = self.validator._run_single_threshold_check(
            flags_in_code, flag_meta_data, flag_data, "1s", "last_traffic_received_at", False
        )
        failed_flag_names = [f["flag"] for f in failed_flags]
        assert "permanent-flag-1" not in failed_flag_names
        assert "permanent-flag-2" not in failed_flag_names
        ...
        failed_flags = self.validator._run_single_threshold_check(
            flags_in_code, flag_meta_data, flag_data, "1s", "last_traffic_received_at", True
        )
        failed_flag_names = [f["flag"] for f in failed_flags]
        assert "permanent-flag-1" not in failed_flag_names
        assert "permanent-flag-2" not in failed_flag_names
